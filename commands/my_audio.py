...
async def my_audio_show(query: CallbackQuery):
    ...
    try:
        ...
        audios = await globals.CompleteCache.user_get_audios(query.from_user.id)
        if len(audios) == 0:
        ...
        for track in audios:
            ...
            markup.add(InlineKeyboardButton(f"{cached['artist']} - {cached['title']} ({convert_from_seconds(cached['duration'])})",
...
async def my_audio_add(query: CallbackQuery):
    ...
    try:
        ...
        if await globals.CompleteCache.user_has_audio(query.from_user.id, int(audio_data[0]), int(audio_data[1])):
            return await query.answer("Эта аудиозапись уже в вашем списке!")

        await globals.CompleteCache.user_add_audio(query.from_user.id, int(audio_data[0]), int(audio_data[1]))
...
async def my_audio_remove(query: CallbackQuery):
    ...
    try:
        ...
        if await globals.CompleteCache.user_has_audio(query.from_user.id, int(audio_data[0]), int(audio_data[1])) is False:
            return await query.answer("Этой аудиозаписи нет в вашем списке!")

        await globals.CompleteCache.user_remove_audio(query.from_user.id, int(audio_data[0]), int(audio_data[1]))
